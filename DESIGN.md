...
- Combined repair scope: All layers keeps the active tool. Erase removes the stroke through every plate, including hidden backing, in one undoable operation. Add / connect takes the topmost cut color at the stroke start and keeps that target for the whole stroke; starting in a transparent gap makes no change. New color samples the original from either preview. Tool help explicitly names the scope; individual-layer editing remains isolated.
